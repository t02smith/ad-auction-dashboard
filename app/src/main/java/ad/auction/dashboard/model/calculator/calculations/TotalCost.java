...
public class TotalCost extends Metric {
    ...
    public Function<Campaign, ArrayList<Point2D>> cumulative(ChronoUnit resolution, boolean isCumulative) {
        return c -> {
            ...
            Future<ArrayList<Point2D>> impCost = executor.submit(() -> Metrics.TOTAL_COST_IMPRESSION.getMetric().cumulative(resolution, isCumulative).apply(c));
            Future<ArrayList<Point2D>> clkCost = executor.submit(() -> Metrics.TOTAL_COST_CLICK.getMetric().cumulative(resolution, isCumulative).apply(c));
