...
public class CTR extends Metric {
    ...
    public Function<Campaign, ArrayList<Point2D>> overTime(ChronoUnit resolution, boolean isCumulative, int factor) {
        return c -> {           
            ...
            Future<ArrayList<Point2D>> impressionPts = executor.submit(() -> Metrics.IMPRESSION_COUNT.getMetric().overTime(resolution, isCumulative, factor).apply(c));
            Future<ArrayList<Point2D>> clickPts = executor.submit(() -> Metrics.CLICK_COUNT.getMetric().overTime(resolution, isCumulative, factor).apply(c));
